...
namespace {
    ...
    public:
        ...
        {
            TC_LOG_INFO("scripts.solocraft.player.instance", "[Frost-Wolf.de] solocraft_player_instance_handler Loaded");
        }
    ...
    private:
        ...
        float CalculateDifficulty(Map *map, Player *player) {
            ...
            if (map) {
                if (map->IsRaid())
                {
                    ...
                    default:
                        TC_LOG_WARN("scripts.solocraft.player.instance", "[Frost-Wolf.de] Unrecognized max players %d, defaulting to 10 man difficulty",
        ...
        void ApplyBuffs(Player *player, Map *map, float difficulty, int numInGroup)
        {
            ...
            if (difficulty != 0)
            {
                ...
                ss << "|cffFF0000[Frost-Wolf.de] |cffFF8000" << player->GetName() << " betritt %s - # mit Spieler: %d - Schwierigkeitsgrad: %0.2f.";
        ...
        void ClearBuffs(Player *player, Map *map)
        {
            ...
            if (unitDifficultyIterator != _unitDifficulty.end())
            {
                ...
                // Inform the player
                std::ostringstream ss;
                ss << "|cffFF0000[Frost-Wolf.de] |cffFF8000" << player->GetName() << " verlässt nach %s - Schwierigkeitsgrad zurückgesetzt: %0.2f.";
